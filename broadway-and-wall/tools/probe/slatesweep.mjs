...
//
// So: N fresh browser profiles, which means N random seeds, across both
// islands. Union of what any town reached, and the per-run spread.
//
//   node tools/probe/slatesweep.mjs [runs-per-island]   # default 3
//
  ...
  for (let i = 0; i < RUNS; i++) {
    ...
    try {
      out = execFileSync("node", [
        join(here, "..", "shoot.mjs"), `${URL_BASE}?bwtown=${town}`, shot,
    ...
    if (!fac || !dck) { console.log(`${town} seed=${seed}  NO READING\n${out.slice(-400)}`); continue; }
    ...
    rows.push({ town, seed, fac: Number(fac[1]), deck: Number(dck[1]) });
    console.log(`${town.padEnd(9)} seed=${String(seed).padStart(10)}  ${String(fac[1]).padStart(3)} facade families  ${dck[1]} roof surfaces`);
  }
}
...
console.log(
  `\n${rows.length} towns · facade families per town: min ${Math.min(...f)} · median ` +
  `${f.slice().sort((a, b) => a - b)[f.length >> 1]} · max ${Math.max(...f)}` +
  `\nunion across all towns: ${facadeUnion.size} facade families, ${deckUnion.size} roof surfaces`,
);
