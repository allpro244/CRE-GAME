...
## 12. OWNER CALLS — LOCKED

Final answers live in `PRINCIPAL_CALLS.md`. Summary:

1. Continue as the heir (phone book resets; bench survives).
2. Own attributes visible; never anyone else's.
3. Fund path optional; balance-sheet default.
4. Fund is a **second cash account** (blocks Phase 6 until buckets land).
5. Estate rate is a world fact; counterplay is the dial (full set).
6. Hire bands never fully close.
7. Firm count bounded by product / deal flow, not headcount.
8. `pnpm staff` frozen through Phase 1; rewrite explicitly at Phase 3.
