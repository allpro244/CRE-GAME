...
export function tickSales(s: GameState, parcels: ParcelTable, adjacency: Adjacency | null = null) {
  for (const h of Object.values(s.holdings)) {
    ...
    // THE ROOM IS NOT ONLY THE TWELVE FIRMS ON THIS STREET. Roughly 45% of the
    // bidders generated by runCallForOffers are pensions, family offices, REITs
    // and other outside capital, yet offer ARRIVAL was driven 100% by the local
    // rivals' dry powder. When those firms were levered or bruised, a correctly
    // priced marketed building waited a measured 20 months against real CRE's
    // 6–12, while the bidder list still claimed outside money existed.
    //
    // Use the same observed split here: local appetite for 55% of the room,
    // the live credit window for 45% of external capital. It remains sharply
    // pro-cyclical—outside buyers disappear when credit closes—but one weak
    // local cohort no longer makes a whole city economically dead.
    const phaseAdj = 0.55 * marketAppetite(s)
      + 0.45 * Math.max(0.2, Math.min(1.2, s.econ.creditIdx ?? 1));
