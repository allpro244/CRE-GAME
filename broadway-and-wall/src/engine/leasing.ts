...
export function genRentRoll(s: GameState, rec: ParcelRecord, holding: Holding) {
  ...
  const anchors = [
    s.quarter + Math.round(rrange(s, 3, 12)),
    s.quarter + Math.round(rrange(s, 13, 30)),
  ];
  let leased = 0;
  let guard = 0;
  while (leased < rec.bldgArea * targetOcc && guard++ < 40) {
    ...
    const endQ = rng(s) < 0.6
      ...
      : s.quarter + Math.round(rrange(s, 2, 32));
    holding.tenants.push({
      ...
      startQ: s.quarter - Math.round(rrange(s, 0, 16)),
      endQ: Math.max(s.quarter + 1, endQ),
    });
    leased += sf;
  }
}
...
export function walt(h: Holding, q: number): number {
  ...
  if (!tot) return 0;
  return h.tenants.reduce((sum, t) => sum + ((t.endQ - q) / 4) * t.sf, 0) / tot;
}
...
export function tickLeasing(s: GameState, parcels: ParcelTable) {
  ...
  for (const h of Object.values(s.holdings)) {
    ...
    if (rec.class === "multifamily") {
      ...
      h.occ = Math.min(0.99, Math.max(0.4, (h.occ ?? target) + (target - (h.occ ?? target)) * 0.3 + rrange(s, -0.015, 0.015)));
      continue;
    }
    if (!isCommercial(rec)) continue;

    const renovating = h.renovatingUntilQ !== undefined && q < h.renovatingUntilQ;

    // move-outs: leases that reached expiry without a signed renewal
    const before = h.tenants.length;
    h.tenants = h.tenants.filter((t) => t.endQ > q);
    ...
    // renewal talks open two quarters ahead of expiry
    for (let i = 0; i < h.tenants.length; i++) {
      const t = h.tenants[i];
      if (t.endQ !== q + 2) continue;
      ...
      s.lois.push({
        ...
        termQ: Math.round(rrange(s, 12, 28)),
    ...
    if (!renovating && vac > 1500 && openLois < 2) {
      ...
      if (rng(s) < p) {
        ...
        s.lois.push({
          ...
          termQ: Math.round(rrange(s, 12, 40)),
          tiPsf: Math.round(rrange(s, tiLo, tiHi)),
          freeQ: Math.round(rrange(s, 0, 2.6)),
...
function leaseCosts(loi: LOI): { ti: number; lc: number } {
  ...
  const lc = loi.rentPsf * loi.sf * (loi.termQ / 4) * (loi.kind === "new" ? 0.04 : 0.02);
...
export function respondLOI(
  ...
  const sign = (l: LOI): string | null => {
    ...
    if (l.kind === "renewal" && l.tenantIdx !== undefined && h.tenants[l.tenantIdx]) {
      ...
      t.rentPsf = l.rentPsf;
      t.endQ = next.quarter + l.termQ;
    } else {
      h.tenants.push({
        name: l.name, sector: l.sector, credit: l.credit,
        sf: l.sf, rentPsf: l.rentPsf, net: l.net,
        startQ: next.quarter, endQ: next.quarter + l.termQ,
        freeUntilQ: l.freeQ ? next.quarter + l.freeQ : undefined,
      });
    }
    next.news.unshift({
      q: next.quarter, kind: "deal",
      text: `Signed: ${l.name} — ${l.sf.toLocaleString()} sf at ${rec.address}, $${l.rentPsf.toFixed(0)}/sf, ${(l.termQ / 4).toFixed(0)} yrs${l.kind === "renewal" ? " (renewal)" : ""}.`,
    });
    return null;
  };
  ...
  if (action === "counter") {
    ...
    next.lois = next.lois.filter((l) => l.id !== id);
    next.news.unshift({ q: next.quarter, kind: "info", text: `${loi.name} walked on the counter at ${rec.address}.` });
